...
- Name: performance.spec.js >> measure checkout flow performance
- Location: tests/performance.spec.js:96:5
...
> 105 |   await page.locator('.btn_inventory').first().click();
  ...
  135 | 
  136 |   // each step should complete within 3 seconds
  137 |   Object.entries(timings).forEach(([step, time]) => {
  138 |     expect(time, `${step} took too long`).toBeLessThan(3000);
  139 |   });
  140 | });
```